...
final class RunBotUITests: XCTestCase {
    ...
    // MARK: - Settings route persistence (#2376)

    /// Verifies that dismissing the panel while Settings is visible (normal close,
    /// no sheet) causes the panel to reopen on Settings rather than Main.
    /// Also verifies that tapping Back explicitly resets the route to Main and
    /// that the reset survives a subsequent close/reopen cycle.
    func testSettingsRouteSurvivesPanelDismissal() {
        openPanel()
        ...
        XCTAssertTrue(
            ...
            "Settings must be visible after tapping Settings button"
        )

        // ── 2. Dismiss via status item (wasForced == false) ──────────────
        let statusItem = app.statusItems.firstMatch
        XCTAssertTrue(statusItem.waitForExistence(timeout: 5), "Status item must exist")
        statusItem.click()
        XCTAssertFalse(
            app.staticTexts["Active local runners"].waitForExistence(timeout: 2),
            "Panel must be closed after toggling status item"
        )

        // ── 3. Reopen — must land on Settings, not Main ──────────────────
        statusItem.click()
        XCTAssertTrue(
            app.staticTexts["Active local runners"].waitForExistence(timeout: 5),
            "Panel must reopen on Settings; savedNavState must be preserved across normal close"
        )
        XCTAssertFalse(
            app.staticTexts["WORKFLOWS"].exists,
            "Main (WORKFLOWS) must not appear on reopen when Settings was the last route"
        )

        // ── 4. Tap Back — route resets to Main ──────────────────────────
        tapButton(app.buttons["Back"])
        XCTAssertTrue(
            app.staticTexts["WORKFLOWS"].waitForExistence(timeout: 5),
            "Main must appear after tapping Settings Back"
        )
        XCTAssertFalse(
            app.staticTexts["Active local runners"].exists,
            "Settings content must not be visible after Back"
        )

        // ── 5. Close/reopen again — must remain on Main ──────────────────
        statusItem.click()
        XCTAssertFalse(
            app.staticTexts["WORKFLOWS"].waitForExistence(timeout: 2),
            "Panel must be closed"
        )
        statusItem.click()
        XCTAssertTrue(
            app.staticTexts["WORKFLOWS"].waitForExistence(timeout: 5),
            "Main must persist across close/reopen after Back was tapped"
        )
        XCTAssertFalse(
            app.staticTexts["Active local runners"].exists,
            "Settings must not reappear after Back reset the route"
        )
    }
