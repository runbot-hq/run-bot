...
extension AddRunnerSheet {
    ...
    /// Returns `true` when the given runner name is already present in the
    /// `localRunners` snapshot injected at the sheet boundary — avoids crossing
    /// the actor boundary in a synchronous computed property.
    func checkDuplicate(runnerName: String) -> Bool {
        localRunners.contains(where: { $0.runnerName == runnerName })
    }
}
